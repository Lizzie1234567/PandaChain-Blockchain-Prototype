...
def selector():
	...
	if request.method == 'POST':
		...
		if node_type == 'node_client':
			bc.my_node = Nodes.Client(Nodes.KeyFactory.load_or_generate_key())
		else:
			bc.my_node = Nodes.Miner(Nodes.KeyFactory.load_or_generate_key())
...
def chat(address: str):
	...
	if request.method == 'POST':
		...
		if text_message == '' and msg_type != -1:
			pass
		...
		if message:
			my_node.send_message(
				...
				total_send=int(request.form.get('blabber_send')) or 0
			)

...
class Client(ANode):
    ...
    def send_message(self, address: str, total_gas: int, total_send: int, message: Messages.AMessage) -> \
        ...
        if total_gas + total_send< Transactions.TransactionOutput.calculate_minimum_blabbers(message):
            return None
        ...
        for utxo in sorted_utxo:
            if allocated_balance >= total_gas + total_send:
                break
        ...
        if total_gas + total_send > allocated_balance:
            return None

        transaction_outputs.append(t_out)
        remaining_balance = allocated_balance - total_gas - total_send
